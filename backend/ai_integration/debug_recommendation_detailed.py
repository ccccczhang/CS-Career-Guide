...
def debug_recommendation_workflow(self_introduction):
    ...
    try:
        ...
        if hasattr(agent, 'steps'):
            print(f"   代理步骤数: {len(agent.steps)}")
            for i, s in enumerate(agent.steps):
                print(f"      Step {i}: {type(s).__name__}")
    ...
    try:
        ...
        print(f"   hasattr tool_calls: {hasattr(result, 'tool_calls')}")
        
        if hasattr(result, 'tool_calls'):
            print(f"   tool_calls 类型: {type(result.tool_calls)}")
            print(f"   tool_calls 内容: {result.tool_calls}")
        ...
        if hasattr(result, 'tool_calls') and result.tool_calls:
            ...
            if hasattr(result, 'content'):
                print(f"   响应内容预览: {result.content[:150] if result.content else '空'}")
            return None
    ...
    print(f"\n📌 Step {step}: 准备第二次调用")
    print("-" * 50)
    print(f"   工具结果数量: {len(all_tool_results)}")
    
    # 构建工具结果消息
    tool_result_content = json.dumps(all_tool_results, ensure_ascii=False, indent=2)
    print(f"   工具结果长度: {len(tool_result_content)} 字符")
    print(f"   工具结果预览: {tool_result_content[:100]}...")
    
    # 添加工具结果消息
    messages.append(AIMessage(content=tool_result_content))
    print(f"   当前消息数量: {len(messages)}")
    
    for i, msg in enumerate(messages):
        print(f"      消息 {i+1}: {type(msg).__name__} - {msg.content[:30]}...")
    step += 1
    ...
    try:
        ...
        print(f"\n✅ 代理调用成功")
        print(f"   返回类型: {type(final_result).__name__}")
        print(f"   hasattr content: {hasattr(final_result, 'content')}")
        print(f"   hasattr tool_calls: {hasattr(final_result, 'tool_calls')}")
        ...
        if hasattr(final_result, 'tool_calls') and final_result.tool_calls:
            print(f"\n   ⚠️ 代理又调用了工具:")
            for tc in final_result.tool_calls:
                print(f"      - {tc.name if hasattr(tc, 'name') else tc.get('name')}")
